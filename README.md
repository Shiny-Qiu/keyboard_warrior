...
- 综合自适应练习：通过本地实时统计和大模型 agent 生成下一批练习词，按 1 到 7 级自动升级、保持、专项加强或降级，并显示当前等级、达标进度和升级要求。
- 实时统计：测试页显示速度、原始速度、准确率和字符统计；练习页按当前监测窗口实时显示速度、原始速度和准确率，单位均为字符/分。
- 结果图表：测试结束后显示速度曲线、原始速度虚线、爆发速度、错误点和悬浮提示。
- 本地成绩：每次测试和英文练习分段都会保存到 `data/records.json`，不限制条数。
- 弱项词：根据错词自动生成弱项词库，供后续练习使用。
- 指法提示：虚拟键盘和双手提示会根据下一个字符高亮对应按键和手指，提示板块可以拖动和复位。
- 错误修正提示：刚打错当前字符时提示使用右手小拇指按 `Delete` 删除；如果继续往后输入，虚拟键盘会继续提示当前下一键，同时提示前面还有错误可回退修正。
...
键盘流畅度不是单纯的“最快一瞬间”，而是速度、错误和修正成本的组合。Gong、Zhang 与 Li 使用 1300 多名中学生的线上写作击键数据研究发现，keyboarding fluency 是一个可靠的个体层面指标；低于某些阈值时，键盘流畅度会更明显影响写作过程和文本表现。项目使用字符/分、原始速度、准确率和错误点，是为了同时观察速度和质量。
...
| 研究结论 | 项目中的对应设计 |
| --- | --- |
| 短时集中练习、速度目标和即时反馈有助于形成打字技能 | 计时测试、实时字符/分、原始速度、准确率 |
...
```bash
screen -dmS jianpanxia5174 sh -lc 'PORT=5174 npm start'
```
...
```bash
screen -S jianpanxia5174 -X quit
```
...
大模型不会逐键实时监控。项目会在本地统计每段练习的速度、原始速度、准确率、错词、慢词、常错键位和手指表现，然后让 agent 返回 `nextWords`、难度决策和一句中文教练反馈。服务端会优先使用当前阶段词库、相似键位词和弱项词，生成 48 到 72 个不重复候选词，再组合成带常见英文标点、数字、撇号和连字符的自然短句。
...
升级只看速度和准确率，不再使用稳定性作为达标条件。连续输入正确且速度较快的词会逐步减少出现；经常打错或偏慢的词、相邻键位词和弱项键位会提高出现频率。
...
- `data/records.json`：每条新记录都会写入中文 `指标` 对象，包含速度、原始速度、准确率、正确字符、错误字符、多余字符和字符统计。
- `稳定性`：当前版本不再作为页面指标、升级条件或记录核心指标。
...
- 调整指法提示逻辑：改 `app.js` 里的 `updateFingerGuide`、`KEYBOARD_LAYOUT` 和 `KEY_FINGERS`。
...
如果刚打错当前字符，指法提示会进入红色修正状态并提示 `Delete`。如果继续往后输入，虚拟键盘会继续高亮当前下一键，并提示前面还有错误可回退修正。拖动过指法提示板块后，可以点击 `复位` 恢复默认位置。
